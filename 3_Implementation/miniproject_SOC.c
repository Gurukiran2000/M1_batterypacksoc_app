...
scanf("%f",&c);
battery_design(v,c);
printf("\nEnter the time duration in minutes for which the vehicle is in initial mode\nt1=");
scanf("%f",&t1);
printf("\nEnter the time duration in minutes for which the vehicle is in dynamic mode\nt2=");
scanf("%f",&t2);
SOC_estimation(t1,t2,Ii,Bq,Id);
}
...
void SOC_estimation(float t1,float t2,float Ii,float Bq,float Id)
{
    ...
    T1=t1/60;
    T2=t2/60;
    SOC=100+(((-Ii/Bq)*T1)+((-Id/Bq)*T2))*100;
    printf("\n\nThe SOC of the battery in percentage is %f\n\n",SOC);
}
